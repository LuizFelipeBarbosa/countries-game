...
const TravleGame = () => {
  ...
  const handleGuess = (guess) => {
    ...
    if (!iso3) {
      setGameState(prev => ({ ...prev, rejects: [...prev.rejects, guess] }));
      return;
    }
      ...
      <div className="flex flex-col md:flex-row gap-4 w-full">
        ...
        <div className="w-full md:w-1/3">
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <p>
              Guesses Remaining: <strong>{gameState.remaining}</strong>
            </p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow mt-4">
            ...
            <ul>
              {gameState.guesses.map(guess => (
                <li key={guess} className={`flex items-center p-1 rounded ${getGuessColor(guess) === 'green' ? 'bg-green-100' : 'bg-yellow-100'}`}>
                  {getGuessColor(guess) === 'green' ? <CheckCircle className="text-green-500 mr-2" /> : <GitCommit className="text-yellow-500 mr-2" />}
      ...
      {gameState.status !== 'playing' && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-8 rounded-lg shadow-lg text-center">
            ...
            <p>
              You found the path in {gameState.guesses.length - 1} guesses.
            </p>
            <p>
              Shortest possible path was {gameState.shortest}.
            </p>
            <button
            ...
            <button
              ...
              onClick={() => {
                const path = gameState.guesses.map(g => countries.find(c => c.alpha3 === g).name[0]).join(' → ');
                const summary = `I completed the Travle from ${countries.find(c => c.alpha3 === gameState.start).name[0]} to ${countries.find(c => c.alpha3 === gameState.end).name[0]} in ${gameState.guesses.length - 1} guesses (shortest ${gameState.shortest}).\n\n${path}`;
