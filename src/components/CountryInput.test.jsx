...
describe('CountryInput', () => {
  ...
  it('calls onSubmit with the input value when the form is submitted', () => {
    ...
    render(<CountryInput onSubmit={handleSubmit} suggestions={['Test Country']} />);
  ...
  it('does not call onSubmit if the input is empty', () => {
    const handleSubmit = vi.fn();
    render(<CountryInput onSubmit={handleSubmit} suggestions={['France']} />);
    const button = screen.getByRole('button', { name: /submit/i });
  ...
  it('disables the input and button when the disabled prop is true', () => {
    render(<CountryInput onSubmit={() => {}} disabled={true} suggestions={[]} />);
    expect(screen.getByPlaceholderText('Enter a country name')).toBeDisabled();
    expect(screen.getByRole('button', { name: /submit/i })).toBeDisabled();
  });

  it('renders suggestions in datalist', () => {
    const { container } = render(<CountryInput onSubmit={() => {}} suggestions={['France', 'Spain']} />);
